...
import dev.thiagooliveira.tablesplit.infrastructure.menu.persistence.projection.ItemProjection;
...
public interface ItemJpaRepository extends JpaRepository<ItemEntity, UUID> {
  ...
  @Query(
      "SELECT NEW dev.thiagooliveira.tablesplit.infrastructure.menu.persistence.projection.ItemProjection("
          + "i, tn, td, KEY(tn)) "
          + "FROM ItemEntity i "
          + "JOIN i.category c "
          + "LEFT JOIN i.name n "
