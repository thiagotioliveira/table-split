package dev.thiagooliveira.tablesplit.infrastructure.notification.api;
...
import dev.thiagooliveira.tablesplit.infrastructure.notification.api.model.SubscriptionData;
  ...
  public ResponseEntity<java.util.List<WaiterCallResponse>> getActiveCalls(Authentication auth) {
    ...
    java.util.List<WaiterCallResponse> response =
        listActiveWaiterCalls.execute(restaurantId).stream()
            .map(
                call ->
                    new WaiterCallResponse(
                        call.getId(), call.getTableCod(), call.getCreatedAt(), call.getCallCount()))
            .toList();
