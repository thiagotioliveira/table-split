...
public class GetReportsOverview {
  ...
  public ReportsOverviewResponse execute(UUID restaurantId, int days) {
    ...
    ZonedDateTime start = end.minusDays(days - 1).withHour(0).withMinute(0).withSecond(0).withNano(0);
    ZonedDateTime prevStart = start.minusDays(days);

    // Fetch orders that were either CLOSED in the period OR are currently OPEN but were opened in the period
    List<Order> orders = new ArrayList<>(
        orderRepository.findAllByRestaurantIdAndStatusAndClosedAtBetween(
            restaurantId, OrderStatus.CLOSED, start, end));
    
    List<Order> openOrders = orderRepository.findAllByRestaurantIdAndStatus(restaurantId, OrderStatus.OPEN);
    orders.addAll(openOrders.stream()
    ...
    response.setOrdersChart(calculateMetricChart(orders, start, days, o -> BigDecimal.valueOf(o.getTickets().size())));
    response.setCustomersChart(calculateMetricChart(orders, start, days, o -> BigDecimal.valueOf(o.getCustomers().size())));
  ...
  private void syncPreviousLabels(ReportsOverviewResponse response, ZonedDateTime start, int days) {
      ...
      for (int i = 0; i < days; i++) {
          ...
          if (response.getRevenueChartPrevious() != null && i < response.getRevenueChartPrevious().size()) {
              response.getRevenueChartPrevious().get(i).setLabel(label);
          }
      }
  }

  private ReportStats calculateStats(List<Order> orders, List<Order> prevOrders, ZonedDateTime start, ZonedDateTime end, ZonedDateTime prevStart) {
    ...
    long totalOrdersCount = orders.stream()
    ...
    int totalParticipants = orders.stream()
        .filter(o -> !o.getTickets().isEmpty() || !o.getPayments().isEmpty())
    ...
    long prevOrdersCount = prevOrders.stream()
        .filter(o -> !o.getTickets().isEmpty() || !o.getPayments().isEmpty())
        .count();
    stats.setOrdersTrend(calculateTrend(BigDecimal.valueOf(totalOrdersCount), BigDecimal.valueOf(prevOrdersCount)));
  ...
  private List<ChartDataPoint> calculateRevenueChart(List<Order> orders, ZonedDateTime start, int days) {
    Map<String, BigDecimal> dailyValues = new TreeMap<>();
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM");
  ...
  private List<ChartDataPoint> calculateMetricChart(List<Order> orders, ZonedDateTime start, int days, Function<Order, BigDecimal> valueExtractor) {
    Map<String, BigDecimal> dailyValues = new TreeMap<>();
    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM");
    ...
    orders.forEach(o -> {
        String key = (o.getClosedAt() != null ? o.getClosedAt() : o.getOpenedAt()).format(formatter);
        if (dailyValues.containsKey(key)) {
  ...
  private List<CategorySaleResponse> calculateCategorySales(List<Order> orders) {
    ...
    orders.stream()
        ...
        .forEach(
            item -> {
              Item domainItem = itemCache.computeIfAbsent(item.getItemId(), id -> itemRepository.findById(id).orElse(null));
              String catName = "Outros";
              if (domainItem != null && domainItem.getCategory() != null) {
                  catName = domainItem.getCategory().getName().get(dev.thiagooliveira.tablesplit.domain.common.Language.PT);
              }
            ...
            item -> {
              ItemSaleResponse res =
                  itemSales.computeIfAbsent(
                      item.getItemId(),
                      id -> {
                        Item domainItem = itemCache.computeIfAbsent(id, itemId -> itemRepository.findById(itemId).orElse(null));
                        ItemSaleResponse r = new ItemSaleResponse();
                        r.setItemId(id);
                        r.setName(item.getName().get(dev.thiagooliveira.tablesplit.domain.common.Language.PT));
                        r.setQuantity(0);
                        r.setTotalRevenue(0.0);
                        if (domainItem != null) {
                            r.setImageUrl(domainItem.getImage());
                            if (domainItem.getCategory() != null) {
                                r.setCategoryName(domainItem.getCategory().getName().get(dev.thiagooliveira.tablesplit.domain.common.Language.PT));
                            }
                        }
                        return r;
                      });
  ...
  private RatingStats calculateRatings(UUID restaurantId, ZonedDateTime since) {
    ...
    stats.setDistribution(
        distribution.entrySet().stream()
            .collect(
                Collectors.toMap(
                    e -> "r" + e.getKey(), e -> e.getValue().intValue())));
